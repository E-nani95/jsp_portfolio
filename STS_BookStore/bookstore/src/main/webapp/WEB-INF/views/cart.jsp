...
<link
	...
	rel="stylesheet">
<%
	String userId = session.getAttribute("usernum").toString();
	System.out.println("============================================");
	 ...
	 <div class="row align-items-md-stretch ">	 	
		...
		<div style="padding-top: 50px">
			...
			<table class="table table-hover">
				...
				<tr>
					<td>${num}</td>
					<td>${order.b_no} - ${order.b_name}</td>
					<td>${order.b_price}</td>
					<td>${order.o_count}</td>
					<td>${order.b_price * order.o_count}</td>
					<td><a href="./orderDelete?id=${order.b_no}" class="badge text-bg-danger">삭제</a></td>
				</tr>
				<c:set var="num" value="${num+1}"/>
				<c:set var="sum" value="${sum+(order.b_price * order.o_count)}"/>
				</c:forEach>
				<tr>
					<th></th>
					<th></th>
